...
AWS.config.update({region: $('#region').children("option:selected").val()});
...
$('#enableTranslationCheckbox').click(function (event) {
    translationEnabled = $("#enableTranslationCheckbox").is(":checked");
});
...
$('#targetLanguage').change(function (event) {
    setTargetLanguage();
});
...
function setTargetLanguage() {
    targetLanguageCode = $('#targetLanguage').find(':selected').val();
}
...
let translateText = function (sourceText) {
    let source = sourceText.Alternatives[0].Transcript;
    ...
    if (typeof source === 'string' || source instanceof String) {
        ...
        translate.config.credentials = {
            accessKeyId: $('#access_id').val(),
            secretAccessKey: $('#secret_key').val()
        };
        translate.config.region = $('#region').children("option:selected").val();
        translate.translateText(params, function (err, data) {
            ...
            if (data) {
                ...
                // fix encoding for accented characters
                // translation = decodeURIComponent(escape(translation));

                $('#transcript').val(translation + "\n");
            }
        });
    }
}
...
function createTranslatePresignedUrl() {
    let endpoint = "translate." + region + ".amazonaws.com:8443";
